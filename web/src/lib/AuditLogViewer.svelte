<script lang="ts">
  ...
  const eventTypes = [
    ...
    { value: 'create_instance', label: 'Create Instance' },
    { value: 'close_instance', label: 'Close Instance' },
    { value: 'attach', label: 'Attach' },
    { value: 'detach', label: 'Detach' },
    { value: 'force_disconnect_agent', label: 'Force Disconnect Agent' },
    { value: 'force_close_instance', label: 'Force Close Instance' },
    { value: 'delete_agent', label: 'Delete Agent' },
    { value: 'add_agent_tag', label: 'Add Tag' },
    { value: 'remove_agent_tag', label: 'Remove Tag' },
  ];
...
<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
  <div class="bg-theme-secondary rounded-lg border border-theme w-full max-w-5xl max-h-[90vh] flex flex-col">
    ...
    <div class="flex items-center justify-between p-4 border-b border-theme flex-shrink-0">
      <div>
        <h2 class="text-lg font-semibold text-theme-primary">Audit Logs</h2>
        <p class="text-xs text-theme-secondary">Total: {total} records</p>
      </div>
      <button
        onclick={onClose}
    ...
    <div class="p-4 border-b border-theme flex-shrink-0">
      <div class="flex items-center gap-4">
        <label class="flex items-center gap-2">
          <span class="text-sm text-theme-secondary">Event Type:</span>
          <select
      ...
      {:else if logs.length === 0}
        <div class="flex items-center justify-center h-full">
          <div class="text-center">
            ...
            <p class="text-theme-secondary text-lg">No audit logs found</p>
          </div>
        </div>
      {:else}
        <table class="w-full text-sm">
          <thead class="text-left text-theme-secondary border-b border-theme">
            <tr>
              ...
              <th class="pb-2 pr-4">Agent ID</th>
              <th class="pb-2 pr-4">Instance ID</th>
              <th class="pb-2 pr-4">Client IP</th>
